...
public class TransferServlet extends HttpServlet {
    protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try (Connection conn = DBUtil.getConnection()) {
            ...
            PreparedStatement findRecipient = conn.prepareStatement("SELECT id FROM user1 WHERE email = ?");
            ...
            PreparedStatement checkBalance = conn.prepareStatement("SELECT balance FROM user1 WHERE id = ?");
            ...
            if (senderRs.next() && senderRs.getDouble("balance") >= amount) {
                PreparedStatement deductSender = conn.prepareStatement("UPDATE user1 SET balance = balance - ? WHERE id = ?");
                deductSender.setDouble(1, amount);
                deductSender.setInt(2, senderId);
                deductSender.executeUpdate();

                PreparedStatement addRecipient = conn.prepareStatement("UPDATE user1 SET balance = balance + ? WHERE id = ?");
                addRecipient.setDouble(1, amount);
                addRecipient.setInt(2, recipientId);
                addRecipient.executeUpdate();

                PreparedStatement logSender = conn.prepareStatement("INSERT INTO transaction(user_id, type, amount, details) VALUES (?, 'Transfer', ?, ?)");
                ...
                PreparedStatement logRecipient = conn.prepareStatement("INSERT INTO transaction(user_id, type, amount, details) VALUES (?, 'Received', ?, ?)");
